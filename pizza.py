...
tag_des_ingredients = ["légume", "viande",
                       "poisson", "fromage", "calorie", "pizza"]

tag_par_ingredient = {"artichaut": "légume", "boeuf haché": "viande", "champignon": "pizza",
...
def est_ingredient_connu(nom):
    if nom in ingredients_connus:
        return True, nom
    elif nom[:-1] in ingredients_connus:
        return True, nom[:-1]
    else:
...
class pizza:
    def __init__(self, nom, ingredients):
        ...
        self.score = [0, 0]
        self.ingredients = []
        self.tags = { tag : [] for tag in tag_des_ingredients}
        for i in ingredients:
            ok, i2 = est_ingredient_connu(i)
            if ok:
                i3 = ingredient(i2)
                self.ingredients.append(i3)
                self.tags[tag_par_ingredient[i2]].append(i3)
            else:
                i3 = ingredient(i2)
                self.ingredients.append(i3)
    ...
    def __str__(self):
        ...
        for tag in self.tags.keys():
            if self.tags[tag]:
                string += "#" + tag + " : "
                string += ", ".join([ingredient.__str__() for ingredient in self.tags[tag]])
                string += "     "
        return string + "\n"
...
def main():
    ...
    scores = [pizza.get_score([], [], ["champignon", "oeuf", "lardon", "artichaut", "olive", "poivron"], []) for pizza in pizzas]
    print("Scores : ", scores)
